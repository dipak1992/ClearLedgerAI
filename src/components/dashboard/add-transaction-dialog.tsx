...
export function AddTransactionDialog({
  ...
  const [transactionType, setTransactionType] = useState<"EXPENSE" | "INCOME" | "TRANSFER" | "REIMBURSEMENT">(
    transaction?.transactionType ?? defaultType ?? "EXPENSE"
  );
  const [transactionDate, setTransactionDate] = useState(
    transaction ? toDateInputValue(transaction.transactionDate) : todayIso()
  );
  ...
  function resetForm() {
    ...
    setTransactionType(transaction?.transactionType ?? defaultType ?? "EXPENSE");
    setTransactionDate(transaction ? toDateInputValue(transaction.transactionDate) : todayIso());
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const endpoint = isEditMode ? `/api/transactions/${transaction.id}` : "/api/transactions";
      ...
      if (!res.ok) throw new Error(data.error ?? `Failed to ${isEditMode ? "update" : "add"} transaction`);
      handleClose();
      router.refresh();
    } catch (err) {
  ...
  return (
    <>
      {triggerContent ? (
        <button
          aria-label={triggerAriaLabel ?? triggerLabel ?? (isEditMode ? "Edit transaction" : "Add transaction")}
        ...
        >
          {triggerLabel ?? (isEditMode ? "Edit Transaction" : "+ Add Transaction")}
        </Button>
      )}
      ...
      {open ? (
        ...
        >
          <div
            className="card-surface w-full max-w-lg rounded-[2rem] p-5 sm:p-7"
            onClick={(e) => e.stopPropagation()}
          >
            <h2 className="text-xl font-semibold">{isEditMode ? "Edit Transaction" : "Add Transaction"}</h2>
            <p className="mt-1 text-sm text-[var(--muted)]">
              {isEditMode ? "Update the details for this record." : "Record an expense, income, or transfer."}
            </p>

            <form className="mt-6 flex flex-col gap-4" onSubmit={handleSubmit}>
              {!isEditMode && workspaces.length > 1 ? (
                <div>
                  <label className="mb-1.5 block text-sm text-white/60">Workspace</label>
                  <select
                    className="min-h-11 w-full rounded-xl border border-white/10 bg-[#111827] px-4 py-2.5 text-sm text-white outline-none focus:border-[var(--brand-500)] focus:ring-1 focus:ring-[var(--brand-500)]"
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {TRANSACTION_TYPES.map((t) => (
                    <button
                      className={`min-h-11 rounded-full px-4 py-1.5 text-sm font-medium transition-all ${
              ...
              <div className="mt-2 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
                ...
                <Button disabled={loading || !title.trim() || !amount} type="submit">
                  {loading ? "Saving…" : isEditMode ? "Save Changes" : "Save Transaction"}
                </Button>
              </div>
            </form>
          </div>
